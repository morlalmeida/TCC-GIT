...
% Find differences between the two curves
diff_y = ReqPow - Pelectric2;

% Find indices where the sign changes (potential crossing points)
crossing_indices = find(diff_y(1:end-1) .* diff_y(2:end) < 0);

% Interpolate to find exact crossing points
x_cross = zeros(size(crossing_indices));
y_cross = zeros(size(crossing_indices));

for i = 1:length(crossing_indices)
    idx = crossing_indices(i);
    x_cross(i) = interp1(diff_y(idx:idx+1), v(idx:idx+1), 0); % Find x where diff_y = 0
    y_cross(i) = interp1(v, ReqPow, x_cross(i)); % Get corresponding y value
end

% Ensure there are at least two crossing points
if length(x_cross) >= 2
    fspeed = x_cross(2);
elseif isscalar(x_cross)
    fspeed = x_cross(1); % Use first crossing if only one exists
else
    fspeed = NaN; % No valid crossing found, assign NaN
end
